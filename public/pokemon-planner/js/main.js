...
// ponytail: vendored at a fixed path, so hardcode instead of sniffing the script tag
// (Astro injects its own module scripts, which would break getScriptParent).
const JS_PATH = "/pokemon-planner/js/";
const IMG_PATH = "/pokemon-planner/img/";

window.onload = buildPage;
// Single-page port: game-picker links only change the hash. replaceState (team
// edits) does NOT fire hashchange, so reloading here only triggers on game select / back.
...
function buildPage() {
    ...
    if ( !currentGame ) {
        // Single-page port: no game in hash → show the game picker.
 ...
 */
 function parseUrl() {
    // Single-page port: game lives in the hash on this same page.
