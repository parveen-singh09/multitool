...
export const clamp = (n: number, min = 0, max = 255) => Math.min(max, Math.max(min, n));
...
export function nearestColorName(rgb: RGB): { name: string; hex: string; exact: boolean } {
  ...
  for (const [name, hex] of Object.entries(CSS_COLORS)) {
    ...
    if (d < bestD) { bestD = d; best = name; bestHex = hex; }
  }
  return { name: best, hex: bestHex, exact: bestD === 0 };
}
